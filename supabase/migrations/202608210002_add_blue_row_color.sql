-- Satır sınıflandırmasına mavi rengi ekler.
alter table public.records
  drop constraint if exists records_row_color_valid;

alter table public.records
  add constraint records_row_color_valid
  check (row_color is null or row_color in ('yellow', 'green', 'red', 'blue'));
...
grant execute on function public.set_record_color(uuid, integer, text)
  to authenticated;
